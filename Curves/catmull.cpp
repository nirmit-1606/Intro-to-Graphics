...
const char *WINDOWTITLE = { "Catmull Curves Animation" };
const char *GLUITITLE   = { "Catmull Curves" };
...
DrawButterfly()
{
	...
	Curve right_anteena = {
		...
		{0., 0., 0.},
		{-1., 0., 0.},
		{-2. + .2 * cos(2. * M_PI * Time), .1, -.5},
		{-1.5, .25, -.8 + .05 * cos(2. * M_PI * Time)}
	...
	Curve right_wing_front = {
		...
		{-1.5 * wing_start.x, -.5 + .3 * cos(4. * M_PI * Time), 1. + .1 * cos(4. * M_PI * Time)},
		wing_start,
	...
	Curve right_leg = {
		...
		{-.3, 0., -.05},
		{-.3, -.1, -.05},
		{-.1, -.5 + .1 * cos(4. * M_PI * Time), -.2},
		{-.1, -.5, -1.5 + .5 * cos(4. * M_PI * Time)}
	};
	Curve left_leg = {
		.6, .3, .1,
		{-.3, 0., .05},
