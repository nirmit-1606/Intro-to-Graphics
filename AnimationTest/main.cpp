...
const char *WINDOWTITLE = { "Lighting Animation" };
...
#define RAD 7.f
	...
	glPushMatrix( );		//white moving light
		SetPointLight( GL_LIGHT0,  (float)(7 + RAD*cos(theta)), 0., (float)(RAD*sin(theta)),  1., 1., 1. );
		if( Light0On )
			glEnable( GL_LIGHT0 );
		else
			glDisable( GL_LIGHT0 );
		...
		glColor3f( 1., 1., 1. );
		glPushMatrix( );
			glTranslatef((float)(7 + RAD * cos(theta)), 0., (float)(RAD * sin(theta)));
	...
	glPushMatrix( );		// red spot light
		SetSpotLight( GL_LIGHT1,  5., 0., 5., -1., 0., 0.,  1., 0., 0. );
		if( Light1On )
			glEnable( GL_LIGHT1 );
		else
			glDisable( GL_LIGHT1 );
	...
	glPushMatrix( );		// purple light
		SetPointLight( GL_LIGHT2,  10., 0., 0., 0.5, 0.5, 1. );
	...
	glPushMatrix( );			// My object
		...
		if(sin(theta) > 0)
			glTranslatef( (float)(-2 * RAD * sin(theta)), 0., (float)(RAD * cos(theta)) );
		else
			glTranslatef( (float)(2 * RAD * sin(theta)), 0., (float)(RAD * cos(theta)) );
	...
	glPushMatrix( );			// Cone
		SetMaterial( 0.7f, 0.7f, 0.1f,  10.f );
		if(sin(theta) > 0)
			glTranslatef( (float)(-2 * RAD * sin(theta)), 1.5, (float)(RAD * cos(theta)) );
		else
			glTranslatef( (float)(2 * RAD * sin(theta)), 1.5, (float)(RAD * cos(theta)) );
		glRotatef(-90., 1., 0., 0.);
		glScalef(2., 2., 2.);
	...
	glPushMatrix( );			// Disc
		SetMaterial( 0.7f, 0.1f, 0.7f,  0.f );
		glTranslatef(0., -8., 0.);
		glScalef(5., 2., 5.);
		glShadeModel( GL_FLAT );
