...
void createDesign(float, float, float, float, float);
...
Display( )
{
	...
	if(WhichView == OUTSIDE) {
		...
		gluLookAt( 25.f, 20.f, 15.f,     0.f, 0.f, 0.f,     0.f, 1.f, -0.2f );
		...
		// set the eye position, look-at position, and up-vector:
		gluLookAt( 0.f, 0.8f, 1.f,     0.f, 5.f, 10.f,     0.f, 1.f, 0.f );

		glutMotionFunc( NULL );

		//stop the movement of propellers
		BladeAngle = 0.;
	...
	//front propeller
	glPushMatrix();
	glTranslatef(0.,1.3,7.5);
	glRotatef(BladeAngle, 0., 0.2, 1.);
	glScalef(5., 1., 1.);
	...
	glPushMatrix();
	glTranslatef(10.,2.3,-0.5);
	glRotatef(BladeAngle * 2, 0., 1., -0.3);
	glScalef(3., 1., 1.);
	glRotatef(90., 1., 0., 0.);
	glCallList(BladeList);
	glPopMatrix();
...
InitLists( )
{
	...
	glBegin( GL_TRIANGLES );
		for( i=0, tp = CESSNAtris; i < CESSNAntris; i++, tp++ )
		{
			...
			Unit( n, n );
			n[1] = fabs( n[1] );
			glColor3f( .3f*n[1], .5f*n[1], .7f*n[1] );
	...
	DesignList = glGenLists(1);
	glNewList(DesignList, GL_COMPILE);

	createDesign(0., 10., 20., 8., 1.); //x,y,z-component, radius, height
	createDesign(0., 10., 20., 7., 1.); //x,y,z-component, radius, height
	createDesign(0., 10., 20., 6., 1.); //x,y,z-component, radius, height
	createDesign(0., 10., 20., 5., 1.); //x,y,z-component, radius, height
	createDesign(0., 10., 20., 4., 1.); //x,y,z-component, radius, height
	createDesign(0., 10., 20., 3., 1.); //x,y,z-component, radius, height
	createDesign(0., 10., 20., 2., 1.); //x,y,z-component, radius, height
	//createDesign(0., 10., 20., 1., 1.); //x,y,z-component, radius, height
...
void createDesign(float dx, float dy, float dz, float radius, float height){
    ...
    angle = 0.0;
        while( angle <= 2*PI ) {
            nx = radius * cos(angle);
            ny = radius * sin(angle);
            glVertex3f(nx, ny + dy, height + nz);
            glVertex3f(nx + 0.3 , ny + dy, nz);
            angle = angle + angle_stepsize;
        }
        glVertex3f(radius, dy, height + nz);
    glEnd();
    ...
    angle = 0.0;
        ...
        glVertex3f(dx, radius + dy, height + nz);
    glEnd();
